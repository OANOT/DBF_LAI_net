...
DATA_PATH = os.path.join(ROOT_DIR, 'data','wheat', 'CloudPointSampleClass')
g_classes = [x.rstrip() for x in open(os.path.join(BASE_DIR, 'meta/wheat_class_names.txt'))]
g_class2label = {cls: i for i,cls in enumerate(g_classes)}
g_class2color = {'wheat':	[0,255,0],
                 'ground':	[0,0,255],
                 'plotMarker':	[255,0,0]}
...
def collect_point_label(anno_path, out_filename, file_format='txt'):
    ...
    Args:
        anno_path: path to annotations. e.g.
    ...
    las = laspy.read(anno_path)
    # 获取点云坐标及颜色
    points = np.vstack((las.x, las.y, las.z)).transpose()
    colors = np.stack([las.red / 65535, las.green / 65535, las.blue / 65535], axis=0).transpose((1, 0))
    labels = np.vstack(las.classification)
    data_label = np.concatenate([points, colors, labels], 1)
